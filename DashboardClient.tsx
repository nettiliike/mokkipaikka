...
import { getOwnerInquiries, saveOwnerContact, type Inquiry } from "@/lib/inquiries";
import Link from "next/link";
import { Plus, Pencil, Trash2, CalendarDays } from "lucide-react";

const empty=(uid:string):Cottage=>({id:crypto.randomUUID(),name:"",city:"",price:100,guests:2,description:"",ownerId:uid,images:[],amenities:{sauna:true},blockedRanges:[],published:true});
export default function DashboardClient(){
 const [user,setUser]=useState<User|null>(null);const [items,setItems]=useState<Cottage[]>([]);const [inquiries,setInquiries]=useState<Inquiry[]>([]);const [editing,setEditing]=useState<Cottage|null>(null);const [message,setMessage]=useState("");const [imageFiles,setImageFiles]=useState<File[]>([]);
 useEffect(()=>onAuthStateChanged(auth,u=>{setUser(u);if(u){getOwnerCottages(u.uid).then(setItems);getOwnerInquiries(u.uid).then(setInquiries).catch(()=>setInquiries([]));if(u.email)saveOwnerContact(u.uid,u.email)}}),[]);
 ...
 return <main className="mx-auto max-w-[1300px] px-5 py-10"><div className="flex items-center justify-between"><div><h1 className="text-4xl font-black">Hallintapaneeli</h1><p className="text-slate-600">Hallitse mökkejä ja kalentereita.</p></div><button onClick={()=>{setImageFiles([]);setEditing(empty(user.uid))}} className="flex gap-2 rounded-xl bg-forest px-5 py-4 font-bold text-white"><Plus/> Lisää mökki</button></div><section className="mt-8 rounded-2xl border bg-white p-5"><h2 className="text-2xl font-black">Yhteydenotot</h2><p className="mt-1 text-sm text-slate-600">Mökkisivun lomakkeella lähetetyt viestit näkyvät tässä.</p>{inquiries.length===0?<p className="mt-4 rounded-xl bg-cream p-4">Ei uusia yhteydenottoja.</p>:<div className="mt-4 grid gap-3">{inquiries.map(i=><article key={i.id} className="rounded-xl border p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-black">{i.cottageName}</h3><p className="font-semibold">{i.name}</p></div>{(i.startDate||i.endDate)&&<p className="text-sm text-slate-600">{i.startDate||"?"} – {i.endDate||"?"}</p>}</div><p className="mt-3 whitespace-pre-line">{i.message}</p><div className="mt-3 flex flex-wrap gap-4 text-sm"><a className="font-bold text-forest" href={`mailto:${i.email}`}>{i.email}</a>{i.phone&&<a className="font-bold text-forest" href={`tel:${i.phone}`}>{i.phone}</a>}</div></article>)}</div>}</section><div className="mt-8 grid gap-5">{items.map(c=><div key={c.id} className="flex flex-col gap-4 rounded-2xl border bg-white p-4 shadow-sm md:flex-row md:items-center"><img src={c.image||c.images?.[0]||"/hero.jpg"} className="h-32 w-full rounded-xl object-cover md:w-48"/><div className="flex-1"><h2 className="text-2xl font-black">{c.name}</h2><p>{c.city} · {c.price} €/yö</p><span className={`mt-2 inline-block rounded-full px-3 py-1 text-sm ${c.published?"bg-green-100 text-green-700":"bg-slate-200"}`}>{c.published?"Julkaistu":"Piilotettu"}</span></div><div className="flex flex-wrap gap-2"><Link href={`/mokki/${c.id}`} className="rounded-lg border px-4 py-3">Näytä</Link><button onClick={()=>{setImageFiles([]);setEditing(c)}} className="flex gap-2 rounded-lg border px-4 py-3"><Pencil size={18}/>Muokkaa</button><button onClick={()=>{setImageFiles([]);setEditing(c)}} className="flex gap-2 rounded-lg border px-4 py-3"><CalendarDays size={18}/>Kalenteri</button><button onClick={async()=>{if(confirm("Poistetaanko ilmoitus?")){await removeCottage(c.id);setItems(items.filter(x=>x.id!==c.id))}}} className="rounded-lg border px-4 py-3 text-red-600"><Trash2 size={18}/></button></div></div>)}</div>{editing&&<div className="fixed inset-0 z-[1500] overflow-y-auto bg-black/45 p-4"><form onSubmit={submit} className="mx-auto my-10 max-w-3xl rounded-3xl bg-white p-7"><div className="flex justify-between"><h2 className="text-3xl font-black">{items.some(x=>x.id===editing.id)?"Muokkaa mökkiä":"Lisää mökki"}</h2><button type="button" onClick={()=>{setImageFiles([]);setEditing(null)}} className="text-2xl">×</button></div><div className="mt-6 grid gap-4 md:grid-cols-2">
